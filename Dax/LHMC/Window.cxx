...
void Window::NextAutoPlayStep()
{
  if(this->AutoPlay)
    ...
    if(demo_step == 1 || demo_step == 3)
      {
      int sign = (fmod(this->CurrentTime,60000.0f) < 30000) ?  1 : -1;
      this->ClipRatio = dax::math::Max(dax::Scalar(-0.98),
                                       this->ClipRatio + (sign * 0.02f) );
      this->ClipRatio = dax::math::Min(dax::Scalar(0.98),
                                       this->ClipRatio);
      }
    this->Remesh = true;
    }
...
void Window::SpecialKey(int key, int daxNotUsed(x), int daxNotUsed(y) )
{
  switch (key)
    ...
    case GLUT_KEY_DOWN :
      ...
      break;
    case GLUT_KEY_LEFT:
      this->ClipRatio = dax::math::Max(-0.98, this->ClipRatio - 0.01);
      this->Remesh = true;
      break;
    case GLUT_KEY_RIGHT:
      this->ClipRatio = dax::math::Min(0.98, this->ClipRatio + 0.01);
      this->Remesh = true;
      break;
    default:
      break;
